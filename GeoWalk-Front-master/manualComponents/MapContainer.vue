...
export default {
  ...
  methods: {
    ...
    renderChart (locations) {
      ...
      this.mapHelper.map.on('pointermove', (e) => {
        this.pointerCoordinates = this.$ol.format.toLonLat(e.coordinate)
        if (this.pointerCoordinates[1] === this.locations[0].latitude) { this.showToolTip = false }
        if (this.pointerCoordinates[0] === this.locations[0].latitude) { console.log('asdasd') }
      })
    },
